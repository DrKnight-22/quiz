...
const CourseDetails = () => {
  ...
  const deleteCourse = async (courseId) => {
    if (
      window.confirm(
        "Are you sure you want to delete this course? This will also delete all associated quizzes and enrollments.",
      )
    ) {
      setLoading((prev) => ({ ...prev, actions: true }))
      ...
      try {
        ...
        await Promise.all([...deletePromises, deleteDoc(doc(db, "courses", courseId))])

        toast.success("Course and related enrollments deleted successfully!")
        await fetchCourses()
  ...
  const handleManageCourse = async (course) => {
    ...
    try {
      ...
      // Error already handled in fetchEnrollments
    }
  }
